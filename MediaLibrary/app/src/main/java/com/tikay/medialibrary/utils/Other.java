...
{ 
	...
	private static void readVoldFile() { 
		 ...
		 * * When one is found, split it into its elements and then pull out 
		 the * path to the that mount point and add it to the arraylist 
	...
	/****	int j = 0; if(sVold.size() > 0) { 
			if(Build.VERSION.SDK_INT < Build.VERSION_CODES.GINGERBREAD) 
				...
				if(!Environment.isExternalStorageRemovable() || Environment.isExternalStorageEmulated()) 
					labelList.add(sContext .getString(R.string.text_internal_storage)); 
				else { labelList.add(sContext .getString(R.string.text_external_storage) + " 1"); 
					j = 1; } } if(sVold.size() > 1) {
				for(int i = 1; i < sVold.size(); i++) { 
					labelList.add(sContext .getString(R.string.text_external_storage) + " " + (i + j)); 
				} 
			} 
		} 
		...
		/* * don't need these anymore, clear the lists to reduce memory use and to * prepare them for the next time they're needed. */ 
		sVold.clear(); 
	}
